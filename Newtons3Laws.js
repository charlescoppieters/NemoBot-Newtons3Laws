   ...
   */
   if (action === '0') {
      // introduction lesson plan
       say(['To understand the way physicists 👨🏾
🔬describe various physical phenomena, they split the types of measurements used into two different categories:', 'Scalars and Vectors']).then(() => {
   ...
   */
   ...
   if (action === 'quiz3') {
    ...
    if (action === '2') {
        ...
        say(['Ok let\’s begin!', {attachment: 'image', url: 'https://i.imgur.com/J8Td9NP.png'},'Hi! My name is Mary Jackson, a former senior engineer 💻 at NASA 🚀 who helped the USA bring many astronauts to space 🌙!','Today, I’m going to be using some of the things I learned from working with rockets 🚀 to teach you about Newton’s Second Law!']).then(() => {sendButton('Continue?', [{title: 'Yes', payload: '{"action" : "1c1"}'}])});
    }
    if (action === '1c1') {
    ...
    if (action === '1c2') {
        say(['Take this rocket 🚀 for example, floating through the frictionless vacuum of space 👩
🚀.', {attachment: 'image', url: 'https://i.imgur.com/JyCHY4q.png'},'Since this rocket 🚀 is floating through empty space, where there is no friction, no gravitational force, and no other internal or external forces, the rocket 🚀 abides by Newton’s First Law: drifting through space at a constant velocity']).then(()=> {sendButton('Continue?', [{title: 'Yes', payload : '{"action" : "1c3"}'}])});
    }
    if (action === '1c3') {
        say('However, imagine the rocket 🚀 hurtling towards the sun ☀️, putting its passengers in danger❌!',{attachment: 'image', url: 'https://i.imgur.com/OjGoYfT.png'}).then(()=> {sendButton('What could the rocket 🚀 do to change its speed and direction to avoid flying straight into the molten hot lava on the sun’s ☀️ surface?', [{title: 'Everyone jump ship', payload : '1c4'},{title: 'Pour water on the sun ☀️', payload : '1c4'},{title: 'Change rocket🚀direction', payload : '{"action" : "1c5"}'}])});
    }
    if (action === '1c4') {
        sendButton('Incorrect 😢. Try the question again!', [{title: 'Try again', payload : '1c3'}, {title: 'Continue', payload : '1c5'}]);
    }
    if (action === '1c5') {
        say(['Great! 😃','The only way the rocket 🚀 can avoid the sun ☀️ is to change directions, which can only be accomplished by acting on the rocket 🚀 with unbalanced forces to override Newton’s First Law.','The rocket 🚀 does this by producing a propulsion force 💥, which we can visualize in a free body diagram like this.', {attachment: 'image', url: 'https://i.imgur.com/g49QRKP.png'}]).then(()=> {sendButton('Continue?', [{title: 'Yes', payload : '{"action" : "1c6"}'}])});
    }
    if (action === '1c6') {
    ...
    if (action === "CharlesQuiz") {
        ...
        if (quiz == 1) {
            say('Good luck!').then(() => {
                sendButton('Question 1: Let\'s say there is a rocket 🚀 flying through space with a net force of 100,000N and a mass of 9500kg. Using Newton\'s Second Law, what is the magnitude of acceleration of the rocket 🚀?', [{title: '9.5e8 m/s^2', payload: JSON.stringify(bPa)}, {title: '10.53 m/s^2', payload: JSON.stringify(gPa)}, {title: '0.095 m/s^2', payload: JSON.stringify(bPa)}]);
            });
        }
        //question 2
        if (quiz == 2) {
            sendButton('Question 2: Let\'s say there is a 300kg rocket 🚀 flying through space with a propulsion force 💥 of 1000N in the north direction and a friction force of 300N in the south direction. What is the direction of the net force of the rocket 🚀?', [{title: 'North', payload : JSON.stringify(gPa)}, {title: 'South', payload : JSON.stringify(bPa)}, {title: 'No acceleration', payload : JSON.stringify(bPa)} ]);
        
        }
        //question 3
        if (quiz == 3) {
            sendButton('Question 3: Using the same rocket 🚀 from the last question, what is the magnitude of acceleration of the rocket 🚀?', [{title: '210,000 m/s^2', payload : JSON.stringify(bPa)}, {title: '0.43 m/s^2', payload : JSON.stringify(bPa)}, {title: '2.33 m/s^2', payload : JSON.stringify(gPa)} ]);
        }
        //question 4
        if (quiz == 4) {
            sendButton('Question 4: What did Mary Jackson do?', [{title: 'Senior engineer at NASA', payload : JSON.stringify(bPa)}, {title: 'Helped win space race', payload : JSON.stringify(bPa)}, {title : 'Incredible mathematician', payload : JSON.stringify(bPa)}, {title: 'All of the above', payload : JSON.stringify(gPa)}]);
        }
        //results
        if (quiz > 4) {
            say(["You scored " + score + " on the quiz.", "The correct answers are as follows:\u000AQ1.  10.53 m/s^2\u000AQ2. North\u000AQ3. 2.33 m/s^2\u000AQ4. All of the above"]).then(() => {
                if (score < 4) {
                    sendButton('You need 3/4 to pass, so now you need to start over!', [{title : 'Try again', payload : '{"action" : "CharlesQuiz", "quiz" : 1, "score" : 0}' }]);
